...
export function Leads() {
  ...
  return (
    <div className="space-y-2">
      ...
      {sheetSyncResult && <div className="flex flex-wrap items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-700"><Check size={14} /><strong>{t('הסנכרון הושלם', 'Sheet sync complete')}</strong><span>{sheetSyncResult.created} {t('נוצרו', 'created')} · {sheetSyncResult.existing} {t('כבר קיימים', 'already existed')} · {sheetSyncResult.excluded ?? 0} {t('נמחקו לצמיתות', 'permanently deleted')} · {sheetSyncResult.skipped} {t('דולגו', 'skipped')}</span>{sheetSyncedAt && <span className="ms-auto text-green-600">{sheetSyncedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}<button onClick={() => setSheetSyncResult(null)}><X size={13} /></button></div>}
